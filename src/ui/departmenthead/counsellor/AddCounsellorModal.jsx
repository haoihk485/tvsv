...
const AddCounsellorModal = ({cb, dataChange}) => {
    ...
    const handleAddCounsellor = () => { 
        dataChange(name, email, phone, password)
    }
            ...
            <div className="flex justify-center h-full items-center ">
                <div className="bg-white rounded-md p-1 flex-col justify-center items-center flex w-[500px]">
                    ...
                    <div className='w-full flex flex-col px-2'>
                        <h1 className='text-center text-lg m-1 text-blue-500 my-2'>Thêm Tư vấn viên</h1>
                        <div className='flex justify-between items-center'>
                            <label htmlFor="username" className='min-w-[150px]'>Tên nhân viên:</label>
                            <MyInput t={'text'} n={'username'} v={name} oC={(e)=>setName(e.target.value)}></MyInput>
                        </div>
                        <div className='flex justify-between items-center mt-2'>
                            <label htmlFor="email" className='min-w-[150px]'>Email:</label>
                            <MyInput t={'email'} n={'email'} v={email} oC={(e)=>setEmail(e.target.value)}></MyInput>
                        </div>
                        <div className='flex justify-between items-center mt-2'>
                            <label htmlFor="phone" className='min-w-[150px]'>Số điện thoại:</label>
                            <MyInput t={'tel'} n={'phone'} v={phone} oC={(e)=>setPhone(e.target.value)}></MyInput>
                        </div>
                        <div className='flex justify-between items-center mt-2'>
                            <label htmlFor="password" className='min-w-[150px]'>Mật khẩu:</label>
                            <MyInput t={'password'} n={'password'} v={password} oC={(e)=>setPassword(e.target.value)}></MyInput>
                        </div>
                    </div>
                    <button className='bg-[#2CC168] rounded-full p-2 m-1 text-white' onClick={handleAddCounsellor}>
